...
def find_initial_fiber_equilibrium(muscle, l_MT, activation):
    """
    ...
    def equilibrium_error(l_m):
        ...
        try:
            ...
            if l_T <= muscle.l_t_slack:
                return 1e6
            
            # Normalized lengths
            l_m_tilde = l_m / muscle.l_m_opt
            l_T_tilde = l_T / muscle.l_t_slack
            ...
            pfl = muscle.curve_pfl.calc_value(l_m_tilde)
            tfl = muscle.curve_tfl.calc_value(l_T_tilde)
    ...
    l_m_min = muscle.l_m_opt * 0.5
    l_m_max = muscle.l_m_opt * 1.5
    
    # For high activation, adjust bounds because muscle will be shorter
    if activation > 0.5:
        l_m_min = muscle.l_m_opt * 0.3  # Allow shorter fibers
        l_m_max = muscle.l_m_opt * 1.2  # Don't need to go as long
    ...
    if result.success and result.fun < 1e-8:
        ...
        if activation > 0.5:
            ...
            l_T_stretched = muscle.l_t_slack * (1.0 + 0.02 * activation)  # 2% stretch per unit activation
            return (l_MT - l_T_stretched) / math.cos(muscle.alpha_m_opt)
        else:
            # Low activation = use your original geometric solution
            alpha_m = muscle.calc_pennation_angle(muscle.l_m_opt)
            l_T = l_MT - muscle.l_m_opt * math.cos(alpha_m)
            if l_T < muscle.l_t_slack:
                l_T = muscle.l_t_slack
                return (l_MT - l_T) / math.cos(alpha_m)
            return muscle.l_m_opt
        
class Simulator:
    def __init__(self, muscle, l_M):
        self.dt = DT
    ...
    def xpbd_substep(self, activation):
        ...
        alpha_m = self.muscle.calc_pennation_angle(self.xpbd_l_M)
        l_T = l_MT - self.xpbd_l_M * math.cos(alpha_m)
        
        # Ensure valid tendon length
        if l_T < self.muscle.l_t_slack:
            l_T = self.muscle.l_t_slack
            self.xpbd_l_M = (l_MT - l_T) / math.cos(alpha_m)
            alpha_m = self.muscle.calc_pennation_angle(self.xpbd_l_M)
        
        # Calculate muscle velocity using your class
        v_m = self.muscle.compute_vel(self.xpbd_l_M, l_MT, activation, alpha_m)
        ...
        loss = self.muscle.muscle_force(v_m, self.xpbd_l_M, activation, alpha_m)/params['fMopt'] + C_values * grad
        print(loss)
    ...
    def classic_substep(self, activation):
        ...
        # Calculate pennation angle and tendon length
        alpha_m = self.muscle.calc_pennation_angle(self.classic_l_M)
        ...
        # Calculate muscle velocity using your class
        v_m = self.muscle.compute_vel(self.classic_l_M, l_MT, activation, alpha_m)
        
        # Calculate muscle force
        f_muscle = self.muscle.muscle_force(v_m, self.classic_l_M, activation, alpha_m) * normalized(dx)
...
def main():
    muscle = Muscle(
        l_m_opt=params['lMopt'], l_t_slack=params['lTslack'], v_m_max=params['vMmax'], alpha_m_opt=params['alphaMopt'], f_m_opt=params['fMopt'],
        beta=params['beta'], a_min=params['aMin'], tau_a=params['tauA'], tau_d=params['tauD']
    )
    
    l_M = find_initial_fiber_equilibrium(muscle, INITIAL_LENGTH, 0.5)
    
    # Initialize the simulator
    simulator = Simulator(muscle, l_M)
    ...
    while t <= 10.0 and not renderer.should_close():
        ...
        activation = 0.0#np.sin(plotter.plot_time * 20) / 2.0 + 0.5
