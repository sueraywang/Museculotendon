...
class Renderer:
    def __init__(self, simulator, width=800, height=600, cam_distance=5.0, MT=False):
    ...
    def _setup_geometry(self):
        ...
        self.spring_lines = [
            Line(),  # XPBD muscle
            Line(),  # XPBD tendon
            Line(),   # Newtonian muscle
            Line()   # Newtonian tendon
        ]
    ...
    def _update_spring_lines(self):
        if not self.MT:
            ...
            vec = self.simulator.particles[1].position - self.simulator.particles[0].position
            direction = vec / np.linalg.norm(vec)
            self.spring_lines[0].update(
                self.simulator.particles[0].position,
                self.simulator.particles[0].position + self.simulator.xpbd_l_M * direction
            )
            self.spring_lines[1].update(
                self.simulator.particles[0].position + self.simulator.xpbd_l_M * direction,
                self.simulator.particles[1].position
            ...
            vec = self.simulator.particles[3].position - self.simulator.particles[2].position
            direction = vec / np.linalg.norm(vec)
            self.spring_lines[2].update(
                self.simulator.particles[2].position,
                self.simulator.particles[2].position + self.simulator.classic_l_M * direction
            )
            self.spring_lines[3].update(
                self.simulator.particles[2].position + self.simulator.classic_l_M * direction,
                self.simulator.particles[3].position
            )
    ...
    def _render_springs(self, view, projection):
        ...
        if not self.MT:
            ...
            glUniform3f(color_loc, 0.0, 1.0, 1.0)
            self.spring_lines[2].draw()
            glUniform3f(color_loc, 1.0, 1.0, 1.0)
            self.spring_lines[3].draw()
