...
params = MuscleParams(
    ...
    alphaMopt=0.1,  # pennation angle at optimal muscle length
...
model_params = {
    ...
    'input_size' : 3,
    'output_size' : 1,
    'model_name' : 'damp_64by3_halfbeta.pth',
    'includes_penn': False,
    'includes_damp': True
}
...
FREE_PARTICAL_INITIAL_VEL = np.array([1.0, 1.0, 1.0]) * 0.3
...
class Particle:
    def __init__(self, position, mass=MASS, fixed=False, xpbd=False):
        self.fixed = fixed
        self.xpbd = xpbd
        self.mass = math.inf if self.fixed else mass
        self.weight = 0.0 if math.isinf(self.mass) else (1.0 / mass)
        
        self.position = position
        self.velocity = np.zeros(3) if self.fixed else FREE_PARTICAL_INITIAL_VEL
...
def find_initial_fiber_equilibrium(l_MT, activation):
    ...
    def equilibrium_error(l_M):
        ...
        # Calculate forces assuming ZERO VELOCITY (fv(0.0) = 1.0)
        ...
        fM_equilibrium = (activation * afl + pfl) * math.cos(alpha_m)
        
        # Force balance error: fiber force should equal tendon force
        return fM_equilibrium - tfl
    
    # Use bracketing search
    l_M_min = params.lMopt * 0.1
    l_M_max = params.lMopt * 2.0
    n_search = 100
    l_M_test = np.linspace(l_M_min, l_M_max, n_search)
    
    # Find where the error function changes sign
    for i in range(len(l_M_test) - 1):
        r1 = equilibrium_error(l_M_test[i])
        r2 = equilibrium_error(l_M_test[i + 1])
        
        # Found a sign change - there's a root here!
        if abs(r1) < 1e6 and abs(r2) < 1e6 and r1 * r2 < 0:
            try:
                solution = brentq(equilibrium_error, l_M_test[i], l_M_test[i + 1], xtol=1e-12)
