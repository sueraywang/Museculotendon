...
class SimulationPlotter:
    ...
    def _setup_plot(self):
        ...
        self.line_x1, = self.ax.plot([], [], 'b-', label="Newtonian Current Length")
    ...
    def update(self, simulator, dt):
        ...
        newtonian_length = np.linalg.norm(simulator.particles[2].position - 
                                        simulator.particles[3].position)
        
        self.xpbd_over_time.append(xpbd_length)
        self.newtonian_over_time.append(newtonian_length)
        self.time_data.append(self.plot_time)
        self.plot_time += dt

        self.line_x.set_data(self.time_data, self.xpbd_over_time)
        self.line_x1.set_data(self.time_data, self.newtonian_over_time)
        
        self.ax.set_xlim(0, max(self.time_data) if self.time_data else 10)
        self.ax.set_ylim(
            min(min(self.xpbd_over_time), min(self.newtonian_over_time)),
            max(max(self.xpbd_over_time), max(self.newtonian_over_time)) 
